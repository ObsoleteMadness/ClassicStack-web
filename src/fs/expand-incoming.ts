/** Unwrap BinHex / MacBinary / StuffIt wrappers dropped into the Finder. */
...
export { isStuffItArchive };

const SIT_TYPES = new Set(['SIT!', 'SIT5', 'SITD']);
const EXPANDABLE_EXTS = new Set(['sit', 'hqx', 'bin']);

/** True when the Finder should offer Expand (`.sit` / `.hqx` / `.bin`, StuffIt type, or BinHex TEXT/SITx). */
export function isExpandableArchive(name: string, finderInfo?: Uint8Array): boolean {
  ...
  return SIT_TYPES.has(type) || (type === 'TEXT' && creator === 'SITx');
}
...
/**
 * If `bytes` is BinHex, MacBinary, or StuffIt (possibly nested), return the inner items.
...
/** Keep unwrapping members that are themselves BinHex / MacBinary / StuffIt. */
...
function sitEntriesToTree(entries: SitEntry[]): ExpandedNode[] {
  ...
  for (const e of entries) {
    ...
    if (e.isFolder) {
      const dir = ensureDir(e.name);
      dir.finderInfo = makeFinderInfo(e.fileType, e.creator, e.finderFlags);
